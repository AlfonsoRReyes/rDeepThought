...
By default, the version of Python found on the system `PATH` is checked first, and then a number of other conventional location for Py Python (e.g. `/usr/local/bin/python`, `/opt/local/bin/python`, etc.) are checked.

## Providing Hints

There are two ways you can provide hints as to which version of Python should be used:

1. By setting the value of the `RETICULATE_PYTHON` environment variable a Python binary.
