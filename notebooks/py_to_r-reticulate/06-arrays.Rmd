...
We have to use the package `reticulate` to be able to see the xpy object in R.

The difference of both arrays should be zero.
...
Python, using row-major ordering, displays the same thing but in a different order where the first indices are grouped together in order. To see that these arrays are, in fact, the same, let's pick out values along just the first "row", that is values with a first index of 1 (R) or 0 (Python):

And this same example using `reticulate`:

```{r}
# array ordering as in "C". row major.
library(reticulate)
np <- import("numpy", convert=FALSE)
apy <- np$reshape(np$arange(1, 25), c(4L, 3L, 2L))
apy
```

```{r}
# array ordering as in "C". row major. explicit
library(reticulate)
np <- import("numpy", convert=FALSE)
cpy <- np$reshape(np$arange(1, 25), c(4L, 3L, 2L), order = "C")
cpy
```

The arrays `apy` and `cpy` are identical.

```{r}
# load the package operator
op <- import("operator", convert = FALSE)
op$sub(apy, cpy)                             # apy - cpy would not work
```
...
## How do we obtain an array just how Python produced in row-major?

### this will not work
```{r}
# it should be zero
r_cm - array(1:24, c(4,3,2))
```

### this will not work either
```{r}
# it should be zero
m <- matrix(1:24, byrow = TRUE)
r_cm - array(m, c(4,3,2))
```

### this works!
```{r}
# this should be zero
# converting R column-major to Python row-major: matrix, array, aperm functions
m <- matrix(1:24, c(3*4,2), byrow = TRUE)
r_cm - aperm(array(m, c(3,4,2)), c(2,1,3))
```

### R function to convert to row-major array

```{r}
r_to_row_major <- function(data = NA, dim = length(data) ) {
    data <- as.vector(data)
    dim <- as.integer(dim)
    dim(data) <- dim
    m <- matrix(data, c(dim[1]*dim[2], dim[3]), byrow = TRUE)
    data <- aperm(array(m, c(dim[2], dim[1], dim[3])), c(2,1,3))
    data
}

r_cm - r_to_row_major(1:24, c(4,3,2))
```
...
```{r}
r_to_row_major(1:8, c(2,2,2))
```

```{r}
# this should give zero
x_cm - r_to_column_major(1:8, c(2,2,2))
```
...
> the last block (dropping the frame) it is exactly the same as the 2nd block in Python.
...
The second array, like Fortran, would be easy to obtain in R because R creates array like Fortran:
...
This contraption would yield row-major, just like C and Python:

```{r}
# row-major
aperm(array(matrix(1:8, c(2*2,2), byrow=T), c(2,2,2)), c(2,1,3))
```
...
```{python}
print(np.reshape(np.arange(1, 25), (4,3,2)))
```
...
```{r}
# Now we take the 2nd index in the third dimension, in Python and R.
np$take(x, 1L, 2L)       #  take(array, index, dimension)
y[, , 2]                 # array[1st_dim, 2nd_dim, 3rd_dim]
```

> The printout is the same, again.
...
# need permutation. Row-major is not natural in R
...
> This is the best way to test if the Python array is identical to R converted array: by subtracting both. If it yields an array of zeros, both are identical.
...
# The `array_reshape` function
The function uses `numpy` to perform the conversion to row-major or column-major using the numpy function `reshape`.

  function (x, dim, order = c("C", "F")) 
  {
      np <- import("numpy", convert = FALSE)
...
x2 <- np$reshape(np$arange(1, 25), c(6L, 4L), "C")
py_to_r(x2)
```
...
x3 = np.reshape(np.arange(1, 25), (6, 4))
print(x3)
```

```{r}
# this will not work, because is column-major order
array(1:24, c(6,4))
```

```{r}
# make an array as row-major
array_reshape(1:24, c(6, 4), order = "C")
```

The function `r_to_row_major` would not work because it was built for 3 dimensions, not two:

```{r}
r_to_row_major(1:24, c(6,4))
```



```{r}
f <- function(data = NA, dim = length(data)) {
    data <- as.vector(data)
    dim <- as.integer(dim)
    print(dim)
    dim(data) <- dim
    # mr <- lapply(seq(1, length(dim)), function(x) dim[x]*dim[x])[[1]]
    mr <- 1
    for (i in 1:(length(dim)-1)) mr <- mr * dim[i]
    # mr <- factorial(length(dim))
    print(mr)
}
...
```{r}
lapply(seq(1, length(dim)-1), function(x) {mr <- 1; mr <- dim[x]*mr})
```

## modified function for 5-D array
...
r_to_row_major <- function(data = NA, dim = length(data) ) {
    ...
    # 5 dimensions
    if (length(dim) == 5) {
      m <- matrix(data, c(dim[1]*dim[2]*dim[3]*dim[4], dim[5]), byrow = TRUE)
      data <- aperm(array(m, c(dim[4], dim[3], dim[2], dim[1], dim[5])), 
                    c(4, 3, 2, 1, 5))
    }       
    # 4 dimensions
    if (length(dim) == 4) {
      m <- matrix(data, c(dim[1]*dim[2]*dim[3], dim[4]), byrow = TRUE)
      data <- aperm(array(m, c(dim[3], dim[2], dim[1], dim[4])), c(3, 2, 1, 4))
    }    
    # 3 dimensions
    if (length(dim) == 3) {
      m <- matrix(data, c(dim[1]*dim[2], dim[3]), byrow = TRUE)
    ...
    # 2 dimensions
    if (length(dim) == 2) {
      m <- matrix(data, c(dim[2] * dim[1]), byrow = TRUE)
      data <- aperm(array(m, c(dim[2], dim[1])), c(2,1))
    }
    data
}
...
```{r}
x4 <- np$reshape(np$arange(1, 121), c(5L, 4L, 3L, 2L), "C")
# print(x4)
x4_to_r <- py_to_r(x4)
x4_to_r
```
...
```{r}
array_reshape(1:120, c(5,4,3,2))
```


```{r}
r_to_row_major(1:120, c(5,4,3,2))
```

```{r}
x4_to_r - r_to_row_major(1:120, c(5,4,3,2))
```

# 5-dimension array

```{r}
x5 <- np$reshape(np$arange(1, 721), c(6L, 5L, 4L, 3L, 2L), "C")
# print(x4)
x5_to_r <- py_to_r(x5)
x5_to_r
```
...
```{r}
array_reshape(1:720, dim = c(6,5,4,3,2), order = "C")
```


```{r}
r_to_row_major(1:720, c(6,5,4,3,2))
```
